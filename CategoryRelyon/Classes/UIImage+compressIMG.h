...
/**
 ...
 @param name icon对应的unicode值，参考KAIcon
 
 @param imageSize image所在视图大小
 
 @param fontSize iconfont大小，建议取imageSize宽高中较小的值
 
 @param color 当前icon颜色
 */
+ (UIImage *)bm_iconWithName:(NSString *)name imageSize:(CGSize)imageSize fontSize:(CGFloat)fontSize color:(UIColor *)color;

/**
 @brief 获取icon，内部调用上面的方法，fontsize，自动取imageSize宽高中较小的值
 */
